...
export async function createAccount(formData: UserRegistrationForm): Promise<{ message: string }> {
    try {
        const url = '/auth/create-account';
        const { data } = await api.post<{ message: string }>(url, formData);
        return data; // Asegura que siempre se retorna un objeto con message
    } catch (error) {
